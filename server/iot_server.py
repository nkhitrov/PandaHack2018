...
class DevicesServer(Server):
    def __init__(self, device: Device, server_lock: Lock, host: str="", port: int=16132, backlog: int=1):
...
class ClientServer(Server):
    def __init__(self, device: Device, server_lock: Lock, host: str="", port: int=13162, backlog: int=5):
        Server.__init__(self, host, port, backlog)
        self._device = device
        self._server_lock = server_lock
        self._thread = Thread(target=self._server_task)
